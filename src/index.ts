...
client.on("ready", async () => {
  ...
  const r = new Discord.RichPresence()
    ...
    .setType("PLAYING")
    .setURL(
      "https://discord.com/channels/994035689471610900/1126279591993028689/1126452522748493845",
    )
    .setName("7Rlk Store")
    .setState("Running...")
    ...
    .setAssetsLargeText("7Rlk Store")
    .setAssetsSmallImage(
      "https://cdn.discordapp.com/attachments/1155836911961788446/1172958285184372927/benner-discord.png",
    )
    .setAssetsSmallText("Join")
    .setStartTimestamp(dateFromTimestamp)
    .addButton("Join", "https://discord.gg/7rlkstore");
...
if (!token) {
  ...
  rl.question(
    gradient(["purple", "pink"])(
      "Insira seu token do Discord (Dica: Quando o token for colocado no .env, não terá mais essa pergunta)\n» ",
    ),
    (input) => {
      if (input.trim() === "") {
        console.log(
          gradient(["red", "orange"])("O token foi retornado como vázio"),
        );
        process.kill(1);
      } else {
        client.login(input).catch((error) => {
          if (error.message === "An invalid token was provided.") {
            ...
            console.log(
              gradient(["red", "orange"])(
                "Esse token não existe (Invalid token)",
              ),
            );
          } else {
            console.clear();
  ...
  client.login(token).catch((error) => {
    ...
    if (error.message === "An invalid token was provided.") {
      console.log(
        gradient(["red", "orange"])("Esse token não existe (Invalid token)"),
      );
    } else {
      console.clear();
      console.error(
        gradient(["red", "orange"])(`Erro ao fazer login: ${error.message}`),
      );
    }
  });
}
...
export const translations: Translations = {
  en: {
    ...
    yandn: " (1 - Yes, 2 - No): ",
    messagesPerChannel:
      "How many messages per channel do you want to clone? (This function is temporarily disabled): ",
    saveToJson: "Do you want to save to JSON? (1 - Yes, 2 - No): ",
    beautifyJson: "Do you want to beautify the JSON? (1 - Yes, 2 - No): ",
    ignoreOptions:
      "Enter what you want to ignore (e.g., emojis, channels, roles): ",
    ...
    ServerID2:
      "Enter your server ID (Server for which you have an administrator role or ownership):",
  ...
  pt: {
    ...
    ServerID2:
      "Digite o ID do seu servidor (Servidor que você tem um cargo administrador ou posse):",
    ...
    messagesPerChannel:
      "Quantas mensagens por canal você deseja clonar? (Esta função está temporariamente desativada): ",
    saveToJson: "Deseja salvar em JSON? (1 - Sim, 2 - Não): ",
    beautifyJson: "Deseja formatar o JSON? (1 - Sim, 2 - Não): ",
    ignoreOptions:
      "Digite o que você deseja ignorar (por exemplo, emojis, canais, cargos): ",
    ...
    error2:
      "Aconteceu um erro (Você pode reportar esse erro em nosso discord):\n",
